...
A skill for production-grade UI design. Supports consistent, scalable frontend development through clear design tokens, layout rules, motion guidelines, and handoff structure.

This skill is the baseline owner for shared tokens, layout rules, and motion guidance. For full accessibility requirements, audits, and edge cases, defer to `web-accessibility` as the canonical source of truth.
...
### Step 4: Coordinate Accessibility Validation

```markdown
## Accessibility Validation Handoff

Use `web-accessibility` as the canonical owner for accessibility requirements. This skill only confirms that the design-system output leaves room for accessible implementation.

### Design-System-Level Checks
- [ ] Text/background: 4.5:1 minimum (normal text)
- [ ] Text/background: 3:1 minimum (large text)
- [ ] UI components: 3:1 minimum
- [ ] Touch targets and spacing can support accessible interaction sizes
- [ ] Motion tokens support reduced-motion fallbacks

### Next Step
- [ ] Run `web-accessibility` for semantic HTML, keyboard support, screen reader behavior, focus order, and ARIA validation
```
