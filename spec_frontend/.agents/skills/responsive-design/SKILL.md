...
:root {
  /* min, preferred, max */
  --font-size-body: clamp(0.875rem, 2.5vw, 1.125rem);
  --font-size-h1: clamp(1.5rem, 5vw, 3rem);
  --font-size-h2: clamp(1.25rem, 4vw, 2.25rem);
}
...
3. **Prefer scalable units**: Use rem, em, %, and fluid viewport/container units for typography and spacing by default
   - font-size: rem or clamp(...rem...)
   - padding/margin: rem or em
   - width: % or vw
   - px remains acceptable for breakpoints, borders, hairlines, and other tokenized fixed thresholds
