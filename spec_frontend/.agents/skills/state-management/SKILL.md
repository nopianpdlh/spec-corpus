...
Redux Toolkit reducers use Immer internally, so mutation-like syntax is acceptable inside `createSlice` reducers. Outside Immer-backed reducers, continue to treat state as immutable.
...
1. **State Immutability**: Do not mutate plain React, Context, or Zustand state directly. Redux Toolkit reducers are the exception because `createSlice` uses Immer under the hood.
    ```tsx
    // ❌ Bad example (plain React/Zustand state)
    state.items.push(newItem);

    // ✅ Good example (plain React/Zustand state)
    setState({ items: [...state.items, newItem] });

    // ✅ Also valid inside Redux Toolkit createSlice reducers
    state.items.push(newItem);
    ```
...
- [web-accessibility](../web-accessibility/SKILL.md): Accessible loading, error, and status messaging in stateful UI
...
function DashboardPage() {
  ...
  if (isError) return <ErrorState onRetry={() => setFilters(filters)} />;

  return <DashboardCharts data={data} onFilterChange={setFilters} />;
}
```
