...
export function Component({
  ...
  className = '',
  ...rest
}: ComponentProps & React.ComponentPropsWithoutRef<'div'>) {
  return (
...
Add DOM ref support only when consumers actually need imperative access. In React 18 that usually means `forwardRef`; in React 19 ref-as-prop patterns are also valid.
...
3. **Accessibility**: Prefer semantic interactive elements first, then add aria-*, role, and tabindex only when semantics alone are not enough. Use `web-accessibility` as the canonical rule set.
...
3. **Inline objects/functions in hot paths**: Avoid them when passing props into memoized children or large lists where they create measurable re-render pressure
   ```tsx
   // ❌ Bad example
...
<Text as="p" className="text-blue-600">Paragraph</Text>
